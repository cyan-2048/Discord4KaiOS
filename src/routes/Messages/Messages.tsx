...
function recursive(_: any) {
	if (!_) return "";
	if (typeof _ === "string") return _;

	if (Array.isArray(_)) {
		return _.map((a) => recursive(a)).join("");
	}

	if (_.type == ":emoji:") return "💀";

	return recursive(_.content);
}
 ...
 */
function testEmojiFont(content: string) {
	return useMemo(() => {
		// fail early
		if (!content) return false;

		const string = recursive(parse(content, {}));
		...
		return filtered.every((a) => a === "💀" || EMOJI_REGEX.test(a));
	}, [content]);
}
...
const Message = memo(function Message({ message, separate, children }: { message: DiscordMessage; children?: ComponentChildren; separate: boolean }) {
	...
	const emojiBig = testEmojiFont(messageContent);
